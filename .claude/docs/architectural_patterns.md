...
- Each service exposes a cheap health route — `/api/health` (server/app/main.py:15),
  ...
  (docker-compose.yml:45, :61, :93; slim images have no curl). `db` uses `pg_isready` (docker-compose.yml:27);
  `graphhopper` uses curl, which its Temurin JRE image ships (docker-compose.yml:74).
- All services: `restart: unless-stopped` (docker-compose.yml:18, :34, :53, :70, :83); dependents wait on
