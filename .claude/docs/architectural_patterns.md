...
- Same shape in server/Dockerfile and ai/Dockerfile: `python:3.12-slim` (:1), install dependencies by reading
...
- Every route is under `/api/...` so a single reverse proxy (Caddy, B10) can split by path:
  `/api/chat` → ai (ai/guardian_ai/api.py:34), the rest → server.
- Each service exposes a cheap health route — `/api/health` (server/app/main.py:15),
  `/api/ai/health` (ai/guardian_ai/api.py:29) — and compose healthchecks call it with the stdlib
  (docker-compose.yml:45, :58; slim images have no curl). `db` uses `pg_isready` (docker-compose.yml:27).
- All services: `restart: unless-stopped` (docker-compose.yml:18, :34, :53); dependents wait on
...
- Health reports `db: "error"` instead of raising (server/app/main.py:22-23).
